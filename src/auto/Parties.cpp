...
static int luasteam_Parties_GetNumActiveBeacons(lua_State *L) {
	...
	uint32 __ret = SteamAPI_ISteamParties_GetNumActiveBeacons(iface);
...
static int luasteam_Parties_GetBeaconByIndex(lua_State *L) {
	...
	PartyBeaconID_t __ret = SteamAPI_ISteamParties_GetBeaconByIndex(iface, unIndex);
...
static int luasteam_Parties_JoinParty(lua_State *L) {
	...
	PartyBeaconID_t ulBeaconID = luasteam::checkuint64(L, 1);
	SteamAPICall_t __ret = SteamAPI_ISteamParties_JoinParty(iface, ulBeaconID);
...
static int luasteam_Parties_GetNumAvailableBeaconLocations(lua_State *L) {
	...
	bool __ret = SteamAPI_ISteamParties_GetNumAvailableBeaconLocations(iface, &puNumLocations);
	lua_pushboolean(L, __ret);
...
// In C++:
// SteamAPICall_t CreateBeacon(uint32 unOpenSlots, const SteamPartyBeaconLocation_t * pBeaconLocation, const char * pchConnectString, const char * pchMetadata);
// In Lua:
// uint64 Parties.CreateBeacon(unOpenSlots: int, pBeaconLocation: SteamPartyBeaconLocation_t, pchConnectString: str, pchMetadata: str, callback: function)
static int luasteam_Parties_CreateBeacon(lua_State *L) {
	auto *iface = SteamParties();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
	...
	uint32 unOpenSlots = static_cast<uint32>(luaL_checkint(L, 1));
	const SteamPartyBeaconLocation_t *pBeaconLocation = lua_isnil(L, 2) ? nullptr : luasteam::check_SteamPartyBeaconLocation_t_ptr(L, 2);
	const char *pchConnectString = luaL_optstring(L, 3, nullptr);
	const char *pchMetadata = luaL_optstring(L, 4, nullptr);
	SteamAPICall_t __ret = SteamAPI_ISteamParties_CreateBeacon(iface, unOpenSlots, const_cast<SteamPartyBeaconLocation_t *>(pBeaconLocation), pchConnectString, pchMetadata);
	if (callback_ref != LUA_NOREF) {
		auto *listener = new luasteam::CallResultListener<CreateBeaconCallback_t>();
		listener->callback_ref = callback_ref;
		listener->call_result.Set(__ret, listener, &luasteam::CallResultListener<CreateBeaconCallback_t>::Result);
	}
	luasteam::pushuint64(L, __ret);
	return 1;
}
...
static int luasteam_Parties_OnReservationCompleted(lua_State *L) {
	...
	SteamAPI_ISteamParties_OnReservationCompleted(iface, ulBeacon, steamIDUser);
	return 0;
}
...
static int luasteam_Parties_CancelReservation(lua_State *L) {
	...
	PartyBeaconID_t ulBeacon = luasteam::checkuint64(L, 1);
	uint64 steamIDUser = luasteam::checkuint64(L, 2);
	SteamAPI_ISteamParties_CancelReservation(iface, ulBeacon, steamIDUser);
	return 0;
}
...
static int luasteam_Parties_ChangeNumOpenSlots(lua_State *L) {
	...
	SteamAPICall_t __ret = SteamAPI_ISteamParties_ChangeNumOpenSlots(iface, ulBeacon, unOpenSlots);
	if (callback_ref != LUA_NOREF) {
...
static int luasteam_Parties_DestroyBeacon(lua_State *L) {
	auto *iface = SteamParties();
	PartyBeaconID_t ulBeacon = luasteam::checkuint64(L, 1);
	bool __ret = SteamAPI_ISteamParties_DestroyBeacon(iface, ulBeacon);
	lua_pushboolean(L, __ret);
	return 1;
}
...
void register_Parties_auto(lua_State *L) {
	...
	add_func(L, "CreateBeacon", luasteam_Parties_CreateBeacon);
