...
namespace {
class UserStatsCallbackListener {
private:
	STEAM_CALLBACK(UserStatsCallbackListener, OnUserStatsReceived, UserStatsReceived_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnUserStatsStored, UserStatsStored_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnUserAchievementStored, UserAchievementStored_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnLeaderboardFindResult, LeaderboardFindResult_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnLeaderboardScoresDownloaded, LeaderboardScoresDownloaded_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnLeaderboardScoreUploaded, LeaderboardScoreUploaded_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnNumberOfCurrentPlayers, NumberOfCurrentPlayers_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnUserStatsUnloaded, UserStatsUnloaded_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnUserAchievementIconFetched, UserAchievementIconFetched_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnGlobalAchievementPercentagesReady, GlobalAchievementPercentagesReady_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnLeaderboardUGCSet, LeaderboardUGCSet_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnGlobalStatsReceived, GlobalStatsReceived_t);
	STEAM_CALLBACK(UserStatsCallbackListener, OnGSStatsUnloaded, GSStatsUnloaded_t);
};
void UserStatsCallbackListener::OnUserStatsReceived(UserStatsReceived_t *data) {
...
void UserStatsCallbackListener::OnUserStatsStored(UserStatsStored_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnUserAchievementStored(UserAchievementStored_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnLeaderboardFindResult(LeaderboardFindResult_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnLeaderboardScoresDownloaded(LeaderboardScoresDownloaded_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnLeaderboardScoreUploaded(LeaderboardScoreUploaded_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnNumberOfCurrentPlayers(NumberOfCurrentPlayers_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnUserStatsUnloaded(UserStatsUnloaded_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnUserAchievementIconFetched(UserAchievementIconFetched_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnGlobalAchievementPercentagesReady(GlobalAchievementPercentagesReady_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnLeaderboardUGCSet(LeaderboardUGCSet_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnGlobalStatsReceived(GlobalStatsReceived_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
void UserStatsCallbackListener::OnGSStatsUnloaded(GSStatsUnloaded_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::UserStats_ref);
...
static int luasteam_UserStats_GetStatInt32(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	int32 pData;
...
static int luasteam_UserStats_GetStatFloat(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	float pData;
...
static int luasteam_UserStats_SetStatInt32(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
...
static int luasteam_UserStats_GetAchievement(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	bool pbAchieved;
...
static int luasteam_UserStats_GetAchievementAndUnlockTime(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	bool pbAchieved;
...
static int luasteam_UserStats_GetAchievementDisplayAttribute(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
...
static int luasteam_UserStats_IndicateAchievementProgress(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
...
static int luasteam_UserStats_RequestUserStats(lua_State *L) {
	auto *iface = SteamUserStats();
...
static int luasteam_UserStats_FindOrCreateLeaderboard(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
	...
	const char *pchLeaderboardName = luaL_optstring(L, 1, nullptr);
...
static int luasteam_UserStats_FindLeaderboard(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
...
static int luasteam_UserStats_DownloadLeaderboardEntries(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
...
static int luasteam_UserStats_DownloadLeaderboardEntriesForUsers(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
	...
	std::vector<CSteamID> prgUsers;
	if (!lua_isnil(L, 2)) {
		luaL_checktype(L, 2, LUA_TTABLE);
		prgUsers.resize(cUsers);
	...
	SteamAPICall_t __ret = SteamAPI_ISteamUserStats_DownloadLeaderboardEntriesForUsers(iface, hSteamLeaderboard, lua_isnil(L, 2) ? nullptr : prgUsers.data(), cUsers);
	if (callback_ref != LUA_NOREF) {
		auto *listener = new luasteam::CallResultListener<LeaderboardScoresDownloaded_t>();
		listener->callback_ref = callback_ref;
		listener->call_result.Set(__ret, listener, &luasteam::CallResultListener<LeaderboardScoresDownloaded_t>::Result);
...
static int luasteam_UserStats_GetDownloadedLeaderboardEntry(lua_State *L) {
	...
	int cDetailsMax = lua_isnil(L, 3) ? 0 : (int)luaL_checkint(L, 3);
	std::vector<int32> pDetails(cDetailsMax);
	bool __ret = SteamAPI_ISteamUserStats_GetDownloadedLeaderboardEntry(iface, hSteamLeaderboardEntries, index, &pLeaderboardEntry, lua_isnil(L, 3) ? nullptr : pDetails.data(), cDetailsMax);
	lua_pushboolean(L, __ret);
...
static int luasteam_UserStats_UploadLeaderboardScore(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
	...
	std::vector<int32> pScoreDetails;
	if (!lua_isnil(L, 4)) {
		luaL_checktype(L, 4, LUA_TTABLE);
		pScoreDetails.resize(cScoreDetailsCount);
	...
	SteamAPICall_t __ret = SteamAPI_ISteamUserStats_UploadLeaderboardScore(iface, hSteamLeaderboard, eLeaderboardUploadScoreMethod, nScore, lua_isnil(L, 4) ? nullptr : pScoreDetails.data(), cScoreDetailsCount);
	if (callback_ref != LUA_NOREF) {
...
static int luasteam_UserStats_AttachLeaderboardUGC(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
...
static int luasteam_UserStats_GetNumberOfCurrentPlayers(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
...
static int luasteam_UserStats_RequestGlobalAchievementPercentages(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
...
static int luasteam_UserStats_GetMostAchievedAchievementInfo(lua_State *L) {
	auto *iface = SteamUserStats();
	uint32 unNameBufLen = lua_isnil(L, 1) ? 0 : (uint32)luaL_checkint(L, 1);
	std::vector<char> pchName(unNameBufLen);
	float pflPercent;
	bool pbAchieved;
	int __ret = SteamAPI_ISteamUserStats_GetMostAchievedAchievementInfo(iface, lua_isnil(L, 1) ? nullptr : pchName.data(), unNameBufLen, &pflPercent, &pbAchieved);
	lua_pushinteger(L, __ret);
...
static int luasteam_UserStats_GetNextMostAchievedAchievementInfo(lua_State *L) {
	auto *iface = SteamUserStats();
	int iIteratorPrevious = static_cast<int>(luaL_checkint(L, 1));
	uint32 unNameBufLen = lua_isnil(L, 2) ? 0 : (uint32)luaL_checkint(L, 2);
	std::vector<char> pchName(unNameBufLen);
	float pflPercent;
	bool pbAchieved;
	int __ret = SteamAPI_ISteamUserStats_GetNextMostAchievedAchievementInfo(iface, iIteratorPrevious, lua_isnil(L, 2) ? nullptr : pchName.data(), unNameBufLen, &pflPercent, &pbAchieved);
	lua_pushinteger(L, __ret);
	lua_pushstring(L, reinterpret_cast<const char*>(pchName.data()));
	lua_pushnumber(L, pflPercent);
	lua_pushboolean(L, pbAchieved);
...
static int luasteam_UserStats_GetAchievementAchievedPercent(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	float pflPercent;
...
static int luasteam_UserStats_RequestGlobalStats(lua_State *L) {
	auto *iface = SteamUserStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
...
static int luasteam_UserStats_GetGlobalStatInt64(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchStatName = luaL_optstring(L, 1, nullptr);
	int64 pData;
...
static int luasteam_UserStats_GetGlobalStatDouble(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchStatName = luaL_optstring(L, 1, nullptr);
	double pData;
...
static int luasteam_UserStats_GetGlobalStatHistoryInt64(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchStatName = luaL_optstring(L, 1, nullptr);
	uint32 cubData = lua_isnil(L, 2) ? 0 : (uint32)luaL_checkint(L, 2);
	std::vector<int64> pData(cubData);
	int32 __ret = SteamAPI_ISteamUserStats_GetGlobalStatHistoryInt64(iface, pchStatName, lua_isnil(L, 2) ? nullptr : pData.data(), cubData);
	lua_pushinteger(L, __ret);
...
static int luasteam_UserStats_GetGlobalStatHistoryDouble(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchStatName = luaL_optstring(L, 1, nullptr);
	uint32 cubData = lua_isnil(L, 2) ? 0 : (uint32)luaL_checkint(L, 2);
	std::vector<double> pData(cubData);
	int32 __ret = SteamAPI_ISteamUserStats_GetGlobalStatHistoryDouble(iface, pchStatName, lua_isnil(L, 2) ? nullptr : pData.data(), cubData);
	lua_pushinteger(L, __ret);
	lua_createtable(L, __ret, 0);
	for(decltype(__ret) i = 0; i < __ret; i++) {
...
static int luasteam_UserStats_GetAchievementProgressLimitsInt32(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	int32 pnMinProgress;
	int32 pnMaxProgress;
...
static int luasteam_UserStats_GetAchievementProgressLimitsFloat(lua_State *L) {
	auto *iface = SteamUserStats();
	const char *pchName = luaL_optstring(L, 1, nullptr);
	float pfMinProgress;
	float pfMaxProgress;
