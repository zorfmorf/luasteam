...
namespace {
class GameServerStatsCallbackListener {
private:
	STEAM_GAMESERVER_CALLBACK(GameServerStatsCallbackListener, OnGSStatsReceived, GSStatsReceived_t);
	STEAM_GAMESERVER_CALLBACK(GameServerStatsCallbackListener, OnGSStatsStored, GSStatsStored_t);
};
void GameServerStatsCallbackListener::OnGSStatsReceived(GSStatsReceived_t *data) {
...
void GameServerStatsCallbackListener::OnGSStatsStored(GSStatsStored_t *data) {
	if (data == nullptr) return;
	lua_State *L = luasteam::global_lua_state;
	if (!lua_checkstack(L, 4)) return;
	lua_rawgeti(L, LUA_REGISTRYINDEX, luasteam::GameServerStats_ref);
...
static int luasteam_GameServerStats_RequestUserStats(lua_State *L) {
	...
	SteamAPICall_t __ret = SteamAPI_ISteamGameServerStats_RequestUserStats(iface, steamIDUser);
...
static int luasteam_GameServerStats_StoreUserStats(lua_State *L) {
	auto *iface = SteamGameServerStats();
	int callback_ref = LUA_NOREF;
	if (lua_isfunction(L, lua_gettop(L))) {
		lua_pushvalue(L, lua_gettop(L));
	...
	uint64 steamIDUser = luasteam::checkuint64(L, 1);
	SteamAPICall_t __ret = SteamAPI_ISteamGameServerStats_StoreUserStats(iface, steamIDUser);
	if (callback_ref != LUA_NOREF) {
